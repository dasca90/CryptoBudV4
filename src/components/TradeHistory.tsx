  ...
  return (
    <div className="trade-list">
      <div className="trade-row trade-header">
        <span>Coin</span><span>Source</span><span>Side</span><span>Mode</span><span>Entry</span><span>Exit</span><span>P&L</span><span>Status</span>
      </div>
      {trades.slice(-50).reverse().map((t, i) => {
        const source = getTradeSourcePresentation(t);
        return (
          <div key={i} className="trade-row" style={{ color: t.pnl && t.pnl > 0 ? '#3fb950' : t.pnl && t.pnl < 0 ? '#f85149' : '#c9d1d9' }}>
            <span>{t.coin}</span>
            <span title={source.fullLabel}>{source.compactLabel}</span>
            <span>{t.side}</span>
            <span>{t.mode}</span>
