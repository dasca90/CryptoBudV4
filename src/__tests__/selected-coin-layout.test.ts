...
ok(selected.includes('setDetailedMode') && selected.includes('Compact') && selected.includes('Detailed'), '3 compact/detailed toggle exists');
ok(selected.includes('onManualBuy(c.symbol)'), '4 buy button remains visible/actionable');
ok(selected.includes('Auto TP reason:') && selected.includes('groupRecommendedStrategy') && selected.includes('effectiveStrategy'), '5 key fields remain visible in source');
...
ok(css.includes('.selected-coin-card') && css.includes('.selected-coin-content') && css.includes('overflow: auto'), '7 internal scroll styling exists');
