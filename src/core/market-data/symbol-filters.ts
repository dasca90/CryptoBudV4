...
export function parseSymbolFilters(symbol: string, exchangeInfo: Record<string, unknown>): SymbolFilters | null {
  try {
    ...
    return {
      ...
      marketMinQty: marketLotSize ? parseFloat(marketLotSize.minQty ?? '0') : primaryLot ? parseFloat(primaryLot.minQty ?? '0') : 0.0001,
      marketMaxQty: marketLotSize ? parseFloat(marketLotSize.maxQty ?? '0') : primaryLot ? parseFloat(primaryLot.maxQty ?? '0') : 1000000,
      marketStepSize: marketLotSize ? parseFloat(marketLotSize.stepSize ?? '0') : primaryLot ? parseFloat(primaryLot.stepSize ?? '0') : 0.0001,
